...
export default function EditAchievementPage() {
  ...
  useEffect(() => {
    ...
    setFormData({
      ...
      date: achievement.publishDate.toISOString().split('T')[0],
      projectId: achievement.projectId || '',
      partnerIds: achievement.partnerIds?.length
      ...
      secondaryColleges: achievement.secondaryColleges || [],
      isPublicDisplay: achievement.isPublicDisplay ?? true,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    achievement.name = formData.title
    achievement.description = formData.description
    achievement.publishDate = new Date(formData.date)
    achievement.projectId = formData.projectId || undefined
    achievement.projectName = formData.projectId
      ? projects.find((p) => p.id === formData.projectId)?.name
      : undefined
    achievement.partnerId = partnerId
    achievement.partnerName = partners.find((p) => p.id === partnerId)?.name || ''
    achievement.partnerIds = formData.partnerIds.length > 0 ? formData.partnerIds : undefined
    achievement.coverImage = formData.coverImage || undefined
    achievement.attachments = formData.attachments.length > 0 ? formData.attachments : undefined
    achievement.secondaryColleges = formData.secondaryColleges.length > 0 ? formData.secondaryColleges : undefined
    achievement.isPublicDisplay = formData.isPublicDisplay
    achievement.status = formData.status
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-foreground">编辑成果</h1>
          <p className="text-sm text-muted-foreground mt-1">修改成果档案信息</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              <CardHeader>
              <CardTitle>成果佐证材料</CardTitle>
              <CardDescription>上传相关成果佐证材料</CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <input
                  type="file"
              ...
              <CardContent>
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setFormData((prev) => ({ ...prev, isPublicDisplay: e.target.checked }))
                    }
                    className="h-4 w-4 rounded border-gray-300 text-primary focus:ring-primary"
                  />
                  <Label htmlFor="isPublicDisplay">在前台展示</Label>
                </div>
              </CardContent>
            </Card>
