...
import type { Job, NamedPhoto, Partner, PartnerType, CooperationStatus, CooperationRating, EnterpriseType, JobCategory } from "@/lib/types"
...
export default function PartnerBrandPage() {
  ...
  // 编辑表单状态（新增表单已提取到 CreatePartnerDialog 组件）
    ...
    { key: "industry", title: "行业", render: (partner: Partner) => partner.industry },
    {
      ...
      render: (partner: Partner) => {
        ...
        const categories = Array.from(
          new Set(partnerJobs.map((job) => job.jobCategory).filter(Boolean))
        ) as JobCategory[]
        if (categories.length === 0) return "-"
        return (
          <div className="flex flex-wrap gap-1">
            {categories.map((cat) => (
              <Badge key={cat} variant="outline">{JOB_CATEGORY_LABELS[cat]}</Badge>
            ))}
          </div>
        )
      },
  ...
  const handleCreate = (newPartner: Partner) => {
    setDisplayedPartners((prev) => {
